...
export default async function ArticleCardTwo() {
  ...
  return (
    <Link key={post._id} href={`/posts/${post.slug}`}>
      <div className="h-full w-full flex flex-col card rounded-none mx-4 my-4 hover:shadow-lg md:hover:shadow-xl md:hover:shadow-slate-700  hover:drop-shadow  hover:bg-base-100 hover:transition-all duration-300 group md:bg-base-200 md:h-full md:w-full md:my-2 md:ml-0 md:mr-3 md:mb-3 md:card-side md:rounded-md">
        ...
        <div className="card-body text-center md:w-full md:absolute md:flex md:p-4 md:bottom-0 md:text-left md:rounded-lg transition-all">
          <div>
            <h1 className="hidden text-3xl font-playfair_display md:flex text-red-800 font-extrabold">
              {post.name}
            </h1>
          </div>

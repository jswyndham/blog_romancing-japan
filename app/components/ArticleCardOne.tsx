    ...
    <Link key={post._id} href={`/posts/${post.slug}`}>
      ...
      <div className="flex flex-col card rounded-none h-full w-full mx-2 my-1 hover:shadow-lg md:hover:shadow-xl md:hover:shadow-slate-700  hover:drop-shadow  hover:bg-gray-50 hover:transition-all duration-300 md:rounded-md md:bg-gray-200 md:flex-row md:my-4 md:mx-0 md:card lg:card-side group">
        ...
        <div className="card-body md:w-[52%] lg:w-[50%] xl:w-[43%] md:absolute md:flex md:p-4 md:right-0 text-center transition-all">
          {/* TITLE @ MD */}
          <div>
            <h2 className="hidden md:flex pb-1 text-4xl font-playfair_display top-0 md:mt-1 lg:mt-2 xl:text-5xl 2xl:mt-6 2xl:pb-2 3xl:pb-4 text-right text-red-800">
              {post.name}
            </h2>
          </div>

          {/* Text summary field */}
          <div className="text-justify text-xl md:text-right md:bottom-0 md:mt-4 md:text-lg lg:mt-12 lg:text-xl">
            <PortableText
              value={post.summary}

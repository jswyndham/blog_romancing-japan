  ...
  return (
    <Link key={post._id} href={`/posts/${post.slug}`}>
      ...
      <div className="flex flex-col card rounded-none h-full w-full mx-2 my-1 hover:shadow-lg md:hover:shadow-xl md:hover:shadow-slate-700  hover:drop-shadow  hover:bg-base-100 hover:transition-all duration-300 md:rounded-md md:bg-base-300 md:flex-row md:my-4 md:mx-0 md:card lg:card-side group">
        ...
        <div className="card-body md:w-[55%] smd:w-[46%] lg:w-[50%] 3xl:w-[37%] md:absolute md:flex md:p-4 md:right-0 text-center transition-all">
          {/* TITLE @ MD */}
          <div>
            <h1 className="hidden md:flex pb-2 text-4xl md:text-5xl font-playfair_display top-0 md:mt-2 lg:mt-2 xl:text-5xl 2xl:mt-16 2xl:pb-4 3xl:pb-8 4xl:mt-1 text-right text-red-800 font-extrabold">
              {post.name}
            </h1>
          </div>

          {/* Text summary field */}
          <div className="text-justify text-xl md:text-right md:bottom-0 md:mt-8 lg:mt-4 xl:mt-12 2xl:mt-1 4xl:mt-4">
            <PortableText
              value={post.summary}
